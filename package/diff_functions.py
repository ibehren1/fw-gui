...
def process_diff(session, request):
    ...
    snapshot_1_list = fix_list(generate_config(session, snapshot=snapshot_1, diff=True)[1])
    snapshot_2_list = fix_list(generate_config(session, snapshot=snapshot_2, diff=True)[1])
